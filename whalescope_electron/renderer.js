...
// Improved loadSectionData function with conditional rendering
async function loadSectionData(section, startDate, endDate) {
  ...
  // Renderizado condicional basado en sección con chequeo de datos válidos
  if (window[`${section}Data`] && !window[`${section}Data`].error) {
    if (section === 'bitcoin') {
      ...
      if (table && window.bitcoinData.markets) {
        ...
        metrics.forEach(metric => {
          if (window.bitcoinData.markets[metric.key] !== undefined) {
            const row = document.createElement('tr');
            row.innerHTML = `<td>${metric.label}</td><td>${metric.format(window.bitcoinData.markets[metric.key])}</td>`;
      ...
      if (lastUpdated && window.bitcoinData.markets?.last_updated) {
        console.log('[Renderer] Updating last update');
        lastUpdated.textContent = new Date(window.bitcoinData.markets.last_updated).toLocaleString();
      }

      if (performanceTable && window.bitcoinData.yields) {
        ...
        periods.forEach(period => {
          if (window.bitcoinData.yields[period.key] !== undefined) {
            const row = document.createElement('tr');
            row.innerHTML = `<td>${period.label}</td><td>${window.bitcoinData.yields[period.key].toFixed(2)}%</td>`;
      ...
      if (topFlowsTable && window.bitcoinData.top_flows) {
        console.log('[Renderer] Updating top flows table');
        topFlowsTable.innerHTML = '';
        window.bitcoinData.top_flows.forEach(flow => {
          const row = document.createElement('tr');
          row.innerHTML = `
            <td>${flow.time}</td>
      ...
      if (flowsTable && (window.bitcoinData.inflows || window.bitcoinData.outflows || window.bitcoinData.net_flow)) {
        ...
        flows.forEach(flow => {
          if (window.bitcoinData[flow.key] !== undefined) {
            const row = document.createElement('tr');
            row.innerHTML = `<td>${flow.label}</td><td>${window.bitcoinData[flow.key].toFixed(2)}</td>`;
      ...
      if (priceTrendChart && window.bitcoinData.price_history) {
        console.log('[Renderer] Updating price trend chart');
        Plotly.purge(priceTrendChart);
        const trace = {
          x: window.bitcoinData.price_history.dates,
          open: window.bitcoinData.price_history.open,
          high: window.bitcoinData.price_history.high,
          low: window.bitcoinData.price_history.low,
          close: window.bitcoinData.price_history.close,
          type: 'candlestick',
      ...
      if (feesChart && window.bitcoinData.fees?.dates && window.bitcoinData.fees?.values) {
        console.log('[Renderer] Updating fees chart');
        Plotly.purge(feesChart);
        const start = new Date(startDate).getTime();
        const end = new Date(endDate).getTime();
        const filteredIndices = window.bitcoinData.fees.dates
          ...
          .map(({ index }) => index);
        const filteredDates = filteredIndices.map(i => window.bitcoinData.fees.dates[i]);
        const filteredValues = filteredIndices.map(i => window.bitcoinData.fees.values[i]);
      ...
      if (analysis && window.bitcoinData.analysis) {
        console.log(`[Renderer] Updating analysis: ${window.bitcoinData.analysis}`);
        analysis.textContent = window.bitcoinData.analysis;
      } else {
        console.log('[Renderer] No analysis data available');
        if (analysis) analysis.textContent = 'No analysis available';
      }

      if (conclusion && window.bitcoinData.conclusion) {
        console.log(`[Renderer] Updating conclusion: ${window.bitcoinData.conclusion}`);
        conclusion.textContent = window.bitcoinData.conclusion;
      } else {
      ...
      if (totalBalance && window.blackrockData.historical_total_balance?.length > 0) {
        console.log('[Renderer] Updating BlackRock total balance');
        const latestBalance = window.blackrockData.historical_total_balance.slice(-1)[0].total_balance_usd;
      ...
      if (totalBalanceTable && window.blackrockData.historical_total_balance?.length > 0) {
        ...
        totalBalanceTable.innerHTML = `
          <table>
            ...
            ${window.blackrockData.historical_total_balance.map(entry => `
              <tr>
      ...
      if (btcBalanceChart && window.blackrockData.historical_balances?.BTC?.length > 0) {
        console.log('[Renderer] Updating BTC balance chart');
        Plotly.purge(btcBalanceChart);
        const trace = {
          x: window.blackrockData.historical_balances.BTC.map(item => item.week_end),
          y: window.blackrockData.historical_balances.BTC.map(item => item.balance_usd),
          type: 'scatter',
          mode: 'lines+markers',
          name: 'BTC Balance (USD)',
          line: { color: '#0d6efd', width: 2 },
          marker: { size: 8, color: '#0d6efd', symbol: 'circle' },
          text: window.blackrockData.historical_balances.BTC.map(item => `$${item.balance_usd.toLocaleString('en-US', { minimumFractionDigits: 2 })}`),
      ...
      if (ethBalanceChart && window.blackrockData.historical_balances?.ETH?.length > 0) {
        console.log('[Renderer] Updating ETH balance chart');
        Plotly.purge(ethBalanceChart);
        const trace = {
          x: window.blackrockData.historical_balances.ETH.map(item => item.week_end),
          y: window.blackrockData.historical_balances.ETH.map(item => item.balance_usd),
          type: 'scatter',
          mode: 'lines+markers',
          name: 'ETH Balance (USD)',
          line: { color: '#28a745', width: 2 },
          marker: { size: 8, color: '#28a745', symbol: 'circle' },
          text: window.blackrockData.historical_balances.ETH.map(item => `$${item.balance_usd.toLocaleString('en-US', { minimumFractionDigits: 2 })}`),
          hovertemplate: '%{text}<br>Date: %{x}<extra></extra>'
        };
      ...
      if (marketStatsTable && window.lidoData.markets?.stETH) {
        ...
        metrics.forEach(metric => {
          if (window.lidoData.markets.stETH[metric.key] !== undefined) {
            const row = document.createElement('tr');
            row.innerHTML = `<td>${metric.label}</td><td>${metric.format(window.lidoData.markets.stETH[metric.key])}</td>`;
      ...
      if (yieldsTable && window.lidoData.yields) {
        console.log('[Renderer] Updating Lido yields table');
        yieldsTable.innerHTML = '';
        const row = document.createElement('tr');
        row.innerHTML = `<td>Average Rewards</td><td>${window.lidoData.yields.avg_rewards.toFixed(2)}%</td>`;
        yieldsTable.appendChild(row);
        console.log('[Renderer] Added row for Average Rewards');
      }

      if (analyticsTable && window.lidoData.analytics) {
        console.log('[Renderer] Updating Lido analytics table');
        analyticsTable.innerHTML = '';
        const row = document.createElement('tr');
        row.innerHTML = `<td>Staking Ratio</td><td>${(window.lidoData.analytics.staking_ratio * 100).toFixed(2)}%</td>`;
        analyticsTable.appendChild(row);
        console.log('[Renderer] Added row for Staking Ratio');
      }

      if (queuesTable && window.lidoData.analytics?.queues) {
        console.log('[Renderer] Updating Lido queues table');
        queuesTable.innerHTML = '';
        window.lidoData.analytics.queues.forEach(queue => {
          const row = document.createElement('tr');
          row.innerHTML = `
      ...
      if (ethStakedChart && window.lidoData.charts?.length > 0) {
        console.log('[Renderer] Updating Lido ETH staked chart');
        Plotly.purge(ethStakedChart);
        const trace = {
          x: window.lidoData.charts.map(item => item.week_end),
          y: window.lidoData.charts.map(item => item.eth_staked),
          type: 'scatter',
          mode: 'lines+markers',
          name: 'ETH Staked',
          line: { color: '#28a745', width: 2 },
          marker: { size: 8, color: '#28a745', symbol: 'circle' },
          text: window.lidoData.charts.map(item => `${item.eth_staked.toFixed(2)} ETH`),
          hovertemplate: '%{text}<br>Date: %{x}<extra></extra>'
        };
    ...
    console.warn(`[Renderer] No valid data for ${section}, skipping render`);
    if (status) status.innerHTML = 'Error: Data not available';
  }
}